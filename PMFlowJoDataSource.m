classdef PMFlowJoDataSource
    ...
    methods % INITIALIZATION
            ...
            function obj = PMFlowJoDataSource(varargin)
                ...
                % 2: PMRowTitles
                % 3: logical scalar: whether or not you want to adjust the means of repeats from experiments;
        ...
        function GroupStatisticsListArray = getGroupStatisticsLists(obj)
            ...
            % it is recommended to not generate groups with semicolons because this causes unncessary complexity;
              ...
              try
                    ...
                    % group-specific data:
                    ListWithPooledSpreadsheets =            obj.getPooledMatrixListsForAllGroups;
                    ListWithGroupIndices =                  obj.getInternalGroupIndicesForAllExternalGroups;
                    ListWithGrourNames =                    obj.getGroupNamesForInternalAndExternalGroups;
                    
                    % data that apply for all groups:
                    MyDescription =                         obj.getDescription;
                    NamesOfAllParameters =                  obj.getRowTitles;
                    ...
                    GroupStatisticsListArray =        cellfun(...
                            @(matrix, indices, names) ...
                                                        PMGroupSpreadsheet(...
                                                                        ...
                                                                        names, ...
                                                                        MyDescription, ...
                                                                        NamesOfAllParameters,...
                                                                        '').getGroupListStatistics, ...
                                                                        ListWithPooledSpreadsheets, ...
                                                                        ListWithGroupIndices, ...
                                                                        ListWithGrourNames...
                                                                    );
...
>>>>>>> Stashed changes
    ...
    methods % GETTERS: MAYBE REMOVED IN FUTURE VERSIONS;
    ...
    methods (Access = private) % ERROR HANDLING FOR USER GENERATED TEXT-FILES;
    ...
    methods (Access = private)  % GETTERS FOR NON-POOLED DATA-SPREADSHEETS (FROM FLOW-JO EXPORTED CSV-FILES);
    ...
    methods (Access = private) % GETTERS FOR POOLED DATA-SPREADSHEETS (FROM FLOW-JO EXPORTED CSV-FILES);

        function Matrices =         getPooledMatrixListsForAllGroups(obj)
    ...
    methods (Access = private) % GETTERS FOR GROUP-INDICES OF INTERNAL GROUPS;

         function GroupRowList = getInternalGroupIndicesForAllExternalGroups(obj)
            % GETGROUPROWS returns cell array with group-indices;
            % each cell contains information for various "external" groups;
            % each cell contains a numerical vector containing indices;

             AllGroupRows =                     obj.GroupIndicesObject.getGroupRows;
        
            GroupRowList =                AllGroupRows(:);
            cellfun(@(x)    assert(isvector(x) && iscell(x), 'Group rows must be a vector of numerical cells.'), GroupRowList);
            cellfun(@(x)   ...
    ...
    methods (Access = private ) % GETTERS: GROUP-NAMES

            function myGroupNames =             getGroupNamesForInternalAndExternalGroups(obj)
                % GETGROUPNAMELIST returns cell array with "group-list";
                % returns a cell-array containing group-names
                % each cell contains information for "external" groups;
                % each cell contains a cell-string array with the different names of the "internal" groups;
                % using external and internal groups together can be confusing;
                % the use of internal groups is recommended
    ...
    methods (Access = private) % GETTERS FOR GENERAL PROPERTIES;
        ...
        function structure =                getStructure(obj)
            ...
            structure.SpreadsheetRowTitles =            obj.getRowTitles;
